...
1. generates a ``crew_run_id`` and builds a configured :class:`~crew_fusebox.state.RunState`;
...
from crew_fusebox.state import RunState, bind_run, unbind_run
...
def _trip_exception(state: RunState) -> CircuitBreakerException:
    ...
    return CircuitBreakerException(
        ...
        recent_traces=list(state.traces),
    )
