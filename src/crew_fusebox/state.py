...
class RunState:
    ...
    # Phase 2: trajectory loop detection (None = disabled).
    trajectory: TrajectoryTracker | None = None
    # Phase 2: last N execution traces for webhook payloads.
    traces: list[str] = field(default_factory=list, repr=False)
    # Phase 2: webhook URL for alarm dispatches (None = disabled).
    ...
    def add_trace(self, trace: str, *, max_traces: int = 3) -> None:
        """Append a human-readable trace string, keeping at most *max_traces*."""
        with self._lock:
...
_active_run: contextvars.ContextVar[RunState | None] = contextvars.ContextVar(
    "crew_fusebox_active_run", default=None
)
